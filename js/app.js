...
let salesChart, expenseChart, chartDays = 7, chartRange = '7', filterStart = null, filterEnd = null, dateMode = 'month', editingTransactionId = null, summaryPeriod = 'daily';
...
function renderDateLabels() { const today = new Date(); const monthStart = new Date(today.getFullYear(), today.getMonth(), 1); const monthEnd = new Date(today.getFullYear(), today.getMonth() + 1, 0); $('#dateRange').textContent = 'Bugün'; $('#currentDate').textContent = new Intl.DateTimeFormat('tr-TR', { day:'numeric', month:'long', year:'numeric', weekday:'long' }).format(today); $('#transactionForm [name="date"]').value = dateInputValue(today); }
...
function totals() {
  ...
  const daily = transactions.filter((item) => sameDay(item.date, now));
  const yesterday = new Date(year, month, now.getDate() - 1);
  const annual = transactions.filter((item) => item.date.getFullYear() === year);
  const calendarMonth = transactions.filter((item) => item.date.getMonth() === month && item.date.getFullYear() === year);
  const previousMonth = transactions.filter((item) => item.date.getMonth() === (month + 11) % 12 && item.date.getFullYear() === (month === 0 ? year - 1 : year));
  ...
  return { revenue, expense, dailyExpense, annualRevenue, annualExpense, calendarMonthRevenue, calendarMonthExpense, cash, card, dailyRevenue, yesterdayRevenue, previousMonthRevenue, previousMonthExpense, annualAverageMonthlyRevenue, profit:revenue-expense };
}
...
function render() { renderKpis(); updateExtendedKpis(); renderSummaryCards(); updateSummaryScrollControls(); renderTable(); renderCharts(); }
...
document.querySelectorAll('#summaryPeriodTabs button').forEach((button) => button.addEventListener('click', () => { summaryPeriod = button.dataset.summaryPeriod; renderSummaryCards(); updateSummaryScrollControls(); })); $('#summaryCards').addEventListener('scroll', updateSummaryScrollControls); $('#summaryScrollPrevious').addEventListener('click', () => $('#summaryCards').scrollBy({ left:-440, behavior:'smooth' })); $('#summaryScrollNext').addEventListener('click', () => $('#summaryCards').scrollBy({ left:440, behavior:'smooth' })); window.addEventListener('resize', updateSummaryScrollControls);
document.querySelectorAll('#analysisTabs button').forEach((button) => button.addEventListener('click', () => { renderAnalysis(button.dataset.analysisPeriod); renderAdditionalAnalysis(button.dataset.analysisPeriod); }));
const summaryCards = $('#summaryCards'); let isSummaryDragging = false; let summaryDragStart = 0; let summaryScrollStart = 0; let summaryLastX = 0; let summaryVelocity = 0; let summaryMomentumFrame;
summaryCards.addEventListener('pointerdown', (event) => { cancelAnimationFrame(summaryMomentumFrame); isSummaryDragging = true; summaryDragStart = event.clientX; summaryLastX = event.clientX; summaryScrollStart = summaryCards.scrollLeft; summaryVelocity = 0; summaryCards.setPointerCapture(event.pointerId); summaryCards.classList.add('is-dragging'); });
summaryCards.addEventListener('pointermove', (event) => { if (!isSummaryDragging) return; const movement = event.clientX - summaryLastX; summaryVelocity = movement; summaryLastX = event.clientX; summaryCards.scrollLeft -= movement; });
summaryCards.addEventListener('pointerup', () => { isSummaryDragging = false; summaryCards.classList.remove('is-dragging'); const glide = () => { summaryVelocity *= 0.94; summaryCards.scrollLeft -= summaryVelocity; if (Math.abs(summaryVelocity) > 0.2) summaryMomentumFrame = requestAnimationFrame(glide); else updateSummaryScrollControls(); }; summaryMomentumFrame = requestAnimationFrame(glide); });
summaryCards.addEventListener('pointercancel', () => { isSummaryDragging = false; summaryCards.classList.remove('is-dragging'); updateSummaryScrollControls(); });
